...
export function Compatibility({ config, setConfig }: { config: Config; setConfig: Dispatch<SetStateAction<Config | null>> }) {
  ...
  const automaticGlobalTool = compatTools.length
    ? defaultWindowsCompatTool(compatTools, config.protonDefaultPolicy)
    : CACHYOS_WINDOWS_COMPAT_TOOL;
  ...
  const activeGlobalTool = !globalTool || globalToolMissing ? automaticGlobalTool : globalTool;
